...
def SimulIA(grille):
    ...
    if liste_possible==[] or gagne(grille)!=0 : return CalculScore(grille)
    Resultats = []
    for coup in liste_possible:
        Grille2 = copy.deepcopy(grille)
        Grille2[coup[0]][coup[1]] = 2
        Score = SimulHumain(Grille2)[1]
        Resultats.append((coup,Score))
    return max(Resultats,key=lambda i:i[1])
...
def SimulHumain(grille):
    ...
    for coup in liste_possible:
        Grille2 = copy.deepcopy(grille)
        Grille2[coup[0]][coup[1]] = 1
        Score = SimulIA(Grille2)[1]
        Resultats.append((coup,Score))
    return max(Resultats,key=lambda i:i[1])
